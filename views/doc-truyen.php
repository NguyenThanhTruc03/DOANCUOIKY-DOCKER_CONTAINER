...
// ==================== LẤY DỮ LIỆU TỪ URL (PHIÊN BẢN MỚI) ====================
$storySlug = isset($_GET['story_slug']) ? $_GET['story_slug'] : '';
$chapterNumber = isset($_GET['chapter']) ? $_GET['chapter'] : '';

if (!isset($_SESSION['viewed'][$storySlug])) {
    $stmt = $conn->prepare("UPDATE truyen SET views = views + 1 WHERE slug = ?");
    $stmt->bind_param("s", $storySlug);
    $stmt->execute();

    $_SESSION['viewed'][$storySlug] = true;
}
...
$chaptersData = $story['chapters'][0]['server_data'] ?? [];
...
foreach ($chaptersData as $chap) {
    if ((string)$chap['chapter_name'] == (string)$chapterNumber) {
        $chapterUrl = $chap['chapter_api_data'];
        break;
    }
}

if (empty($chapterUrl)) {
    echo "Không tìm thấy chapter " . htmlspecialchars($chapterNumber);
    exit;
}

// Gọi API lấy nội dung chapter
$ch = curl_init();
...
$chapterName = $chapterItem['chapter_name'] ?? $chapterNumber;
...
// Tìm index để lấy chap trước / chap sau
$currentIndex = -1;
foreach ($chaptersData as $index => $chap) {
    if (trim($chap['chapter_name']) === trim($chapterNumber)) {
        $currentIndex = $index;
        break;
    }
}

$prevChapter = ($currentIndex > 0) ? $chaptersData[$currentIndex - 1]['chapter_name'] : null;
$nextChapter = ($currentIndex < count($chaptersData) - 1) ? $chaptersData[$currentIndex + 1]['chapter_name'] : null;
?>
...
<head>
    ...
    <link href="img/logo.png" rel="icon">  <!-- Đã sửa: bỏ ../ -->
...
<body class="bg-gray-900 text-white dark-mode min-h-screen transition-all duration-300">
    <?php include(__DIR__ . '/../includes/header.php'); ?>  <!-- Đã sửa: bỏ ../ -->
        ...
        <div class="content-wrapper max-w-5xl mx-auto">
            ...
            <div class="chapter-info mb-5">
                ...
                <div class="text-gray-400 text-sm mb-2">
                    <a href="index.php" class="hover:underline">Trang Chủ</a> /  <!-- Đã sửa -->
                ...
                <div class="bg-gray-100 text-gray-700 px-4 py-2 rounded-lg flex justify-between items-center">
                    ...
                    <div class="space-x-2">
                        <button id="prevChapterBtn" <?php echo $nextChapter ? '' : 'disabled'; ?>
                                onclick="window.location.href='doc-truyen.php?story_slug=<?php echo urlencode($storySlug); ?>&chapter=<?php echo urlencode($prevChapter); ?>'"                                class="bg-blue-500 text-white px-4 py-1 rounded hover:bg-blue-600 transition disabled:bg-gray-400 disabled:cursor-not-allowed">
                            ← Chap trước
                        </button>
                        <button id="nextChapterBtn" <?php echo $nextChapter ? '' : 'disabled'; ?>
                                onclick="window.location.href='doc-truyen.php?story_slug=<?php echo urlencode($storySlug); ?>&chapter=<?php echo urlencode($nextChapter); ?>'"
            ...
            <div class="chapter-navigation text-center mb-16 bg-gray-900 p-4 rounded-lg">
                ...
                <div class="flex justify-center space-x-3">
                    <button id="prevChapterNav" <?php echo $nextChapter ? '' : 'disabled'; ?>
                            onclick="window.location.href='doc-truyen.php?story_slug=<?php echo urlencode($storySlug); ?>&chapter=<?php echo urlencode($nextChapter ?? ''); ?>'"
                    ...
                    <button id="nextChapterNav" <?php echo $nextChapter ? '' : 'disabled'; ?>
                            onclick="window.location.href='doc-truyen.php?story_slug=<?php echo urlencode($storySlug); ?>&chapter=<?php echo urlencode($nextChapter ?? ''); ?>'"
                            class="bg-blue-500 text-white px-4 py-2 rounded hover:bg-blue-600 transition disabled:bg-gray-400 disabled:cursor-not-allowed">
                        Chương sau →
                    </button>
    ...
    <?php include(__DIR__ . '/../includes/footer.php'); ?>  <!-- Đã sửa: bỏ ../ -->

    <!-- Phần còn lại giữ nguyên (thanh điều hướng, modal, script) -->
    ...
    <div id="bottomNavigation" class="bottom-navigation fixed bottom-0 left-1/2 transform -translate-x-1/2 bg-gray-800 p-2 flex justify-center items-center z-[1000] shadow-[0_-2px_5px_rgba(0,0,0,0.7)] rounded-lg">
        ...
        <button id="backChapter" <?php echo $nextChapter ? '' : 'disabled'; ?>
                onclick="window.location.href='doc-truyen.php?story_slug=<?php echo urlencode($storySlug); ?>&chapter=<?php echo urlencode($prevChapter ?? ''); ?>'"
        ...
        <button id="nextChapter" <?php echo $nextChapter ? '' : 'disabled'; ?>
                onclick="window.location.href='doc-truyen.php?story_slug=<?php echo urlencode($storySlug); ?>&chapter=<?php echo urlencode($nextChapter ?? ''); ?>'"
                class="bg-blue-600 text-white text-sm px-3 py-1 mx-1 rounded hover:bg-blue-700 transition disabled:bg-gray-500 disabled:cursor-not-allowed">
            >
        </button>
...
<div id="chapterModal" class="modal hidden fixed inset-0 bg-black bg-opacity-80 z-[2000]">
    <div class="modal-content absolute top-1/2 left-1/2 transform -translate-x-1/2 -translate-y-1/2 bg-gray-800 p-5 rounded-lg max-h-[80vh] overflow-y-auto w-11/12 max-w-sm text-white relative">
        ...
        <button id="closeModal" 
            class="absolute top-2 right-2 text-red-500 text-xl font-bold hover:text-red-400 transition">
        </button>
        ...
        < id="chapterList">
            ...
            <?php
            // Sắp xếp chương giảm dần
            usort($chaptersData, function($a, $b) {
            ...
            foreach ($chaptersData as $index => $chapter) : ?>
                ...
                onclick="selectChapter('<?php echo urlencode($storySlug); ?>','<?php echo htmlspecialchars($chapter['chapter_name']); ?>')">
    ...
    function selectChapter(storySlug, chapterNumber) {
        window.location.href =
            "doc-truyen.php?story_slug=" + encodeURIComponent(storySlug)
