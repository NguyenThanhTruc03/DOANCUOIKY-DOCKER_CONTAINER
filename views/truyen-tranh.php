...
<script>
    ...
    const chapterHtml = `
        <div class="chapter-item bg-gray-700 p-3 rounded mb-2 hover:bg-gray-600 transition cursor-pointer"
             data-chapter="${chapter.name}">

            <span class="chapter-name text-white">
                Chương ${chapter.name}
            </span>

            <span class="chapter-details text-sm text-gray-400">
    ...
    function saveToReadHistory(chapterUrl, storySlug, chapterName) {
        ...
        const chapterInfo = {
            ...
            chapter_link: `/views/doc-truyen.php?story_slug=${encodeURIComponent(storySlug)}&chapter=${encodeURIComponent(chapterName)}`,
   ...
   $(document).ready(function() {
    ...
    // ==================== PHẦN BÌNH LUẬN (giữ nguyên submit) ====================
    ...
    // ==================== PHẦN ĐỌC CHƯƠNG (đã sửa) ====================

    // ==================== XỬ LÝ CLICK CHƯƠNG - PHIÊN BẢN MỚI (CHỈ TRUYỀN SỐ CHƯƠNG) ====================

// Click vào từng chương
// Click vào từng chương
$('#chapterContainer').on('click', '.chapter-item', function() {

    let chapterName = $(this).data('chapter');

    let storySlug = "<?php echo htmlspecialchars($comicData['slug']); ?>";

    window.location.href =
        "doc-truyen.php?story_slug=" +
        encodeURIComponent(storySlug) +
        "&chapter=" +
        encodeURIComponent(chapterName);
});
// Đọc từ chương đầu tiên
$('#readFirstChapter').on('click', function() {
    if (chapters.length === 0) return;
    let chapterName = chapters[0].name;
    let storySlug = "<?php echo htmlspecialchars($comicData['slug']); ?>";
    
    saveToReadHistory(chapterName, storySlug, chapterName);
    
    window.location.href = `doc-truyen.php?story_slug=${encodeURIComponent(storySlug)}&chapter=${encodeURIComponent(chapterName)}`;});

// Đọc từ chapter mới nhất
